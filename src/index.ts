...
const SUMMARIZED_DESCRIPTIONS: Record<string, string> = {
  browser_click: 'Click a page element (by `ref`) or viewport coordinates in an exactly-bound tab. Default `trusted` route = hardware-like `Input.dispatchMouseEvent`, refuses where it cannot preserve standalone-browser background posture; `input_route="dom_event"` (synthetic `el.click()`, ref required) only when explicitly requested — proves dispatch, not control activation.',
  ...
  browser_pointer: 'Perform hover / right-click / double-click / scroll / drag in an exactly-bound browser tab. Semantic refs must declare `pointer` for hover/right-click/double-click/drag; scroll accepts a `scroll` or `pointer` capability. `trusted` route uses CDP Input events, refuses if standalone background posture cannot be preserved; `dom_event` route requires a page ref.',
  ...
  type_text: 'Insert text into the target pid via character-by-character `PostMessage(WM_CHAR)` to the focused window. No focus steal.',
  scroll: 'Scroll the focused region of the target pid.',
  get_window_state: 'Walk a running app UIA tree and return BOTH a structured `elements` array (preferred) AND a Markdown rendering (back-compat). Every actionable element is tagged `[element_index N]` in the markdown and as `element_index` in the structured array — pass those indices to `click`, `type_text`, `scroll`, etc.',
}
