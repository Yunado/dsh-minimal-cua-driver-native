...
const DROP_TOOLS = new Set([
  'move_cursor', 'set_agent_cursor_theme', 'set_agent_cursor_motion', 'set_agent_cursor_enabled', 'get_agent_cursor_state',
  'end_session', 'get_session', 'get_session_state', 'list_sessions', 'escalate_session',
  'start_recording', 'stop_recording', 'get_recording_state', 'install_ffmpeg', 'replay_trajectory',
])
...
const CONDENSE_SHORT: Record<string, string> = {
  session: 'Repeat a consistent label; omit → implicit. Browser targets/tabs/refs bind to it.',
  delivery_mode: 'background = mandatory first attempt; foreground only after background_unavailable.',
  element_index: 'Needs matching snapshot_id (stale fails closed); prefer element_token.',
  ...
  input_route: "trusted (default) won't foreground a standalone browser; dom_event = synthetic DOM click.",
  window_id: 'HWND scoping the element cache; from list_windows/target. Optional once element_token carries it.',
  ...
  from_zoom: 'true right after a zoom — x/y then refer to the last zoom image; auto-mapped back to window coords.',
  scope: 'window (default, needs pid/window_id) vs desktop (screen-absolute, no pid).',
}
...
const SUMMARIZED_DESCRIPTIONS: Record<string, string> = {
  browser_click: 'Click a page element (by `ref`) or viewport coordinates in an exactly-bound tab. Refused for heuristic bindings. Default `trusted` route = hardware-like `Input.dispatchMouseEvent`, refuses where it cannot preserve standalone-browser background posture; `input_route="dom_event"` (synthetic `el.click()`, ref required) only when explicitly requested — proves dispatch, not control activation.',
  browser_type: 'Type text into an exactly-bound tab via the Input domain. `mode="insert_text"` (default) = `Input.insertText`; `mode="keystrokes"` = per-character key events. Both insert at the caret (typing into a field with text appends); `replace=true` sets/clears the field. Pass a `ref` to an editable element from the latest snapshot. Dispatch is not acceptance: trusted/CDP input only guarantees the event was delivered, not that the field took it (controlled components / IME / some editors can drop it), so a `success` result is not proof the value landed — read back the field (re-`get_browser_state`) to confirm.',
...
function injectCondenseShort(node: unknown): unknown {
  ...
  if (node !== null && typeof node === 'object') {
    ...
    if (properties && typeof properties === 'object' && !Array.isArray(properties)) {
      for (const [field, def] of Object.entries(properties as Record<string, unknown>)) {
...
export async function apply(ctx: Context): Promise<void> {
  ...
  async function mountRuntime(inner: Context): Promise<void> {
    const { CuaDriver, RuntimeAuthorizationOptions } = await import('@trycua/cua-driver')
    ...
    const authHost = {
      ...
      async authorize(request) {
        return { action: 0 /* DriverAuthorizationAction.Allow */, requestDigest: request.requestDigest }
      }
    }
    const activeDriver = driver = CuaDriver.createConfiguredWithAuthorizationHost(
      { claudeCodeCompatibility: false, authorization: RuntimeAuthorizationOptions.create({ allowedModes: [0, 1, 2], compatibilityMode: 0, unrestrictedAcknowledged: true, maxSessionTtlSeconds: 3600n, maxIdleTtlSeconds: 1800n }) },
      authHost,
    ) as NativeDriver
    ...
    for (const tool of catalog.tools) {
      ...
      const definition = createMcpToolDefinition(inner, {
        name: publicName,
        rawName: tool.name,
        description: CONDENSE_PROSE ? (SUMMARIZED_DESCRIPTIONS[tool.name] ?? tool.description ?? '') : (tool.description ?? ''),
        inputSchema: CONDENSE_PROSE ? (injectCondenseShort(stripSchemaProse(tool.inputSchema)) as Record<string, unknown>) : tool.inputSchema,
        ...
        async call(args, execution) {
          ...
          combined.throwIfAborted()
          const parsed = JSON.parse(result.rawJson) as { content?: Array<{ type?: string; text?: string }>; structuredContent?: unknown }
          ...
          if (sc !== undefined && sc !== null) {
            ...
            if (structured.length > 4000) structured = `${structured.slice(0, 4000)} …(truncated)`
            const suffix = `\n\n[structuredContent]\n${structured}`
            const contentBlocks = Array.isArray(parsed.content) ? parsed.content : []
            const textBlock = [...contentBlocks].reverse().find((b) => typeof b.text === 'string')
            if (textBlock !== undefined) {
              textBlock.text = `${textBlock.text}${suffix}`
            } else {
              parsed.content = [{ type: 'text', text: `[structuredContent]\n${structured}` }, ...contentBlocks]
            }
          }
